...
from .models import Course, StudentCourse, CourseHold, TrainerAssignment, StudyMaterial, StudyMaterialFile, StudentFeedback, FeedbackAttachment, CourseHoldHistory
    ...
    def get_bbb_join_url(self, obj):
        ...
        if request and request.user and obj.is_group_class:
           try:
             ...
             logger.info(f"BBB Room found for course {obj.id}: {room.room_id}")
             return room.get_join_url(request.user)
           except BigBlueButtonRoom.DoesNotExist:
            logger.warning(f"No BBB Room found for course {obj.id}")
        return None
...
class StudentCourseSerializer(serializers.ModelSerializer):
    ...
    def get_bbb_join_url(self, obj):
        try:
            ...
            # Determine if the user is a moderator or attendee
            is_moderator = user.role in ['ADMIN', 'MANAGER', 'TRAINER']
            password = 'mp' if is_moderator else 'ap'  # moderator password or attendee password
            ...
            # Generate checksum
            checksum_string = f"join{urlencode(sorted(join_params.items()))}{settings.BBB_SECRET}"
            checksum = hashlib.sha1(checksum_string.encode('utf-8')).hexdigest()

            # Construct full join URL
            join_url = f"{settings.BBB_URL}api/join?{urlencode(join_params)}&checksum={checksum}"
